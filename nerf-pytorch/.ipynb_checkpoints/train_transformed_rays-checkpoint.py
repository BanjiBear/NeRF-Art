...
def main():
    ...
    if train_background:
        with torch.no_grad():
            avg_img = torch.mean(images[i_test],axis=0)
            # Blur Background:
            if blur_background:
    ...
    if fixed_background: # load GT background
        ...
        print("should be ", images[i_test][0].shape)
        assert background.shape == images[i_test][0].shape
    else:
        background = None
    ...
    for i in trange(start_iter, cfg.experiment.train_iters):
        ...
        if USE_CACHED_DATASET:
            ...
            img_idx = np.random.choice(i_test)
