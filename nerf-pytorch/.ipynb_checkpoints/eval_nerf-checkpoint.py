...
def main():
    ...
    if cfg.dataset.type.lower() == "blender":
        ...
        images, poses, bds, render_poses, i_test = load_llff_data(
            cfg.dataset.basedir, factor=cfg.dataset.downsample_factor, spherify=True
        )
        hwf = poses[0, :3, -1]
        H, W, focal = hwf
    ...
    render_poses = render_poses.float().to(device) #Guy Changed to render test images
    ...
    for i, pose in enumerate(tqdm(render_poses)):
        ...
        with torch.no_grad():
            ...
            rgb_coarse, disp_coarse, _, rgb_fine, disp_fine, _, _ = run_one_iter_of_nerf(
                hwf[0],
                hwf[1],
                hwf[2],
                model_coarse,
