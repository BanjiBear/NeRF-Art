...
def render_path_spiral(c2w, up, rads, focal, zdelta, zrate, rots, N):
    ...
    for theta in np.linspace(0.0, 2.0 * np.pi * rots, N + 1)[:-1]:
        c = np.dot(
            c2w[:3, :4],
            np.array([0.5*np.cos(theta), -0.5*np.sin(theta), -np.sin(theta * zrate/2), 1.0])
            * rads,
        )
...
def load_llff_data(
    basedir, factor=4, recenter=True, bd_factor=0.75, spherify=False, path_zflat=False
):
    #factor = None
    #spherify = True
    []
